...
- [ ] 탐색 페이지의 게시물 URL을 제한 없이 중복 제거하는 파서 테스트와 구현
- [ ] 게시물 HTML의 article 작성자 username 추출 테스트와 구현

### Checkpoint

- [ ] `test_public_parser.py` 통과

### Phase 2: 목표 기반 Worker

- [ ] 랜덤 hashtag/post 순회 generator 구현
- [ ] 10계정을 넘어 탐색하고 새 후보 목표에서 멈추는 Worker 테스트와 구현
- [ ] 시간·차단·소스 소진·선택적 비상 한도 종료 경로 구현

### Checkpoint

- [ ] `test_public_main.py`와 전체 Worker 테스트 통과
- [ ] `compileall` 통과

### Phase 3: 실행·검증

- [ ] Preview 전용 랜덤 원샷 PowerShell wrapper 추가
- [ ] 환경변수와 운영 경계를 README에 기록
- [ ] 실제 Playwright 탐색과 Preview 자동 수집 검수 결과 증거 저장
