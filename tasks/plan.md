...
- [ ] 가격 200000·홈 배너 false의 Admin Edge → DB → Admin list → Public REST 검증
- [ ] 랭킹 category/sort/period/cursor의 실제 RPC/Edge 일관성 검증
- [ ] 알림 실패·빠른 중복 탭·빈값/오류/no-mock 회귀 테스트 보강
- [ ] 홈 → 캘린더 → 상세, 랭킹 → 마이 → 상세 공용 Maestro 여정 추가
- [ ] Reels blur/background/back/100 swipes와 Android sheet/back 회귀 흐름 추가
