...
- [ ] Android 실행·녹화 증거와 플랫폼 공용 재현 절차 기록
- [ ] 전체 테스트·린트·타입·빌드, 독립 리뷰, PR CI와 main CI 완료
...
- [ ] Query client 기본 정책·AppState focus bridge·query error 관측을 RED/GREEN으로 구현
- [ ] PostgREST GET no-store 헤더와 화면별 retry override 제거
- [ ] GNB/sticky duplicate 접근성 제외와 배너 자동 회전 접근성 선호 반영
- [ ] DealCard의 상품명·가격·판매자·마감 label 보강
- [ ] 캘린더 fontScale 기반 row/card 높이와 scroll offset 연결
- [ ] 랭킹 상위 카드·일반 행·필터를 큰 글꼴에서 확장 가능한 구조로 변경
- [ ] 캘린더·랭킹·마이·제보 Modal의 제목·격리·announcement 적용
- [ ] 공통 stale/empty/error/retry UI를 홈·캘린더·랭킹·검색·Reels에 적용
- [ ] 광범위 LogBox ignore 제거, 전체 테스트·Android TalkBack/큰 글꼴 런타임 증거 검증
- [ ] 위키·Linear·PR·CI·merge·main 동기화
