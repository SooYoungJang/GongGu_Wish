...
## Ranking Discovery Redesign v2 Implementation Plan

기존 GON-260의 기능 계약은 유지하면서, 기술 설명과 필터가 상품보다 먼저 보이는 문제를 해소한다. 서버 인기점수를 현재 목록 기준 0~100 인기지수와 근거 문구로 번역하고, 상위 3위와 이후 순위에 명확한 시각 위계를 적용한다. 상세·알림 action, Dynamic Type, dark mode와 비동기 상태는 기존 계약을 그대로 재사용한다.

### Decisions

- DB/API/score weight는 변경하지 않고 기존 `deepViews`, `bookmarks`, `notifications`, `searchClicks`, `score`, `scoreDelta`를 presentation에 사용한다.
- 1위 hero, 2·3위 spotlight, 4위 이후 compact card를 별도 역할로 유지하되 공통 인기지표 helper를 공유한다.
- 큰 글꼴 또는 좁은 화면에서는 2·3위를 세로로 전환하고 고정 card height를 사용하지 않는다.
- `RankingBasisBar`는 기술 명세 카드가 아니라 한 줄 설명과 업데이트 상태만 남긴다.

### Phases

1. 인기지수 정규화·근거 문구 helper를 RED/GREEN으로 구현
2. 상위 3위 image-led presentation을 RED/GREEN으로 재설계
3. 4위 이후 compact card와 목록 virtualization/action을 보강
4. StoreScreen의 compact sticky filter와 상태 화면을 통합
5. Android 접근성·다크·좁은 화면 E2E, 위키 증거, review와 배송

### Risks and Mitigations

| Risk | Impact | Mitigation |
| --- | --- | --- |
| 필터가 다시 상품을 viewport 아래로 밀어냄 | High | basis를 한 줄로 축소하고 360px 초기 viewport screenshot을 수용 기준에 포함 |
| 2열 카드가 320px·큰 글꼴에서 잘림 | High | width/fontScale 기반 세로 fallback, 고정 높이 금지 |
| 인기지수가 실제 구매량처럼 오해됨 | High | `인기지수`와 신호 근거만 표시하고 구매 카피 금지 |
| heterogeneous card가 list 성능을 저하시킴 | Medium | 안정 callback, memoized item, 기존 가상화 목록과 작은 응답 크기 유지 |

### Checkpoints

- [x] Presentation: helper·TopThree·row targeted tests 통과
- [x] Integration: StoreScreen와 mobile 전체 test/typecheck/lint/build 통과
- [x] Runtime: Android 일반/큰 글꼴·dark·320px와 상세 동작 증거 확보
- [ ] Delivery: 위키, code review, PR/main CI, local main fast-forward 완료
