# Spec: Development, Preview, and Production Environments

## Objective

Separate GongGu Wish into three explicit release lanes so developers can iterate quickly, two internal testers can verify a production-like app, and store users receive only approved production releases.

The desired installed variants are:

| Variant     | Display name     | Application ID            | Backend    | EAS channel   |
| ----------- | ---------------- | ------------------------- | ---------- | ------------- |
| Development | 공구위시 Dev     | `com.gonggu.wish.dev`     | staging    | `development` |
| Preview     | 공구위시 Preview | `com.gonggu.wish.preview` | staging    | `preview`     |
| Production  | 공구위시         | `com.gonggu.wish`         | production | `production`  |

Development connects to a local Metro server. Preview and Production run without a local server and only receive updates from their own EAS Update channel.
...
- Development build: `npx eas-cli build --platform android --profile development`
...
- Keep production as the safe default when `APP_VARIANT` is absent.
...
- Validate each resolved Expo config with `expo config` using `APP_VARIANT=development|preview|production`.
- Run existing mobile unit tests, lint, and TypeScript checks.
- Smoke-test staging Worker health and a real allowlisted Supabase query.
...
- Always: keep Production identifiers and backend unchanged; use migrations as the schema source; use test-only data in staging; keep EAS channels isolated.
...
- All three variants resolve to distinct names, schemes, and application identifiers.
- Development and Preview use staging Supabase and `api-staging.gongguwish.com`; Production remains on the existing production services.
- Development and Preview builds can coexist with the Production app on one device.
...
- Kakao and Naver developer credentials are required before those providers can
  be enabled in staging. The current Production Supabase project also reports
