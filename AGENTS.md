...
5. 머지된 정확한 SHA의 Preview DB·Edge Functions·API Worker·관리자 웹·모바일과 실제 smoke test가 `Preview Green`을 통과할 때까지 작업을 완료로 간주하지 않는다.
6. Preview 실패를 우회하거나 건너뛰어 `main`에 반영하지 않는다.
...
5. `main` 머지 뒤 Production DB migration, RLS, Edge Functions, Cloudflare Worker, 관리자 웹, 모바일 Build/OTA 결과와 실제 Production identity를 끝까지 확인한다.
