...
import { fetchMyGroupBuyRequests, setMyRequestNotification } from "./myRequestsApi";

const request = vi.hoisted(() => vi.fn());
vi.mock("../../lib/postgrest-client", () => ({ postgrestFetch: request }));
const row = (id: string) => ({ request_id: id, product_name: "요청한 상품", status: "OPEN", requested_at: "2026-09-11T00:00:00.000Z", group_buy_id: null, notification_enabled: false });
describe("my request pages", () => {
  ...
  it("uses an account guard and the last visible row as the next cursor", async () => {
    ...
    expect(request).toHaveBeenCalledWith("rpc/list_my_group_buy_requests_v2", { method: "POST", body: {
  ...
  it("guards the account when changing notification consent and requires acknowledgement", async () => {
    request.mockResolvedValue({ data: true });
    await expect(setMyRequestNotification("owner", "request", true)).resolves.toBe(true);
    expect(request).toHaveBeenCalledWith("rpc/set_my_request_notification", { method: "POST", body: { p_expected_user_id: "owner", p_request_id: "request", p_enabled: true } });
    request.mockResolvedValue({ data: null });
    await expect(setMyRequestNotification("owner", "request", false)).rejects.toThrow();
  });
});
