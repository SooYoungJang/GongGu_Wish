...
export async function fetchMyGroupBuyRequests(userId: string, cursor: MyRequestCursor | null = null): Promise<MyRequestPage> {
  const { data } = await postgrestFetch<unknown>("rpc/list_my_group_buy_requests", {
    method: "POST",
  ...
  const rows = data.map((row: unknown): MyGroupBuyRequest => {
    ...
    return { id: value.request_id, productName: value.product_name, status: value.status as MyGroupBuyRequest["status"], requestedAt: value.requested_at };
  });
