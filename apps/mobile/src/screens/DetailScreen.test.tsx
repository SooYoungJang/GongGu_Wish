...
vi.mock("@shopify/flash-list", () => {
  ...
  return {
    FlashList: ReactMock.forwardRef(
      (
        { data = [], renderItem, keyExtractor, children, ...props }: any,
        ref: any,
      ) => {
...
vi.mock("react-native-pager-view", () => {
  ...
  const PagerView = ReactMock.forwardRef(
...
vi.mock("react-native", () => {
  ...
  return {
    ...
    Share: { share: vi.fn() },
    StatusBar: ({ ...props }: any) =>
      ReactMock.createElement("StatusBar", props),
...
vi.mock("../components/AppButton", () => ({
  AppButton: ({ children, onPress, variant, style, disabled }: any) => {
    ...
    return ReactMock.createElement(
      "Pressable",
      { onPress, style, variant, disabled },
      children,
    );
  },
}));
...
function getTransformValue(styles: any[], key: string) {
  for (const style of styles) {
    ...
    const entry = transform.find(
      (item: Record<string, number>) => item[key] !== undefined,
    );
...
function getMediaStageFrame(renderer: TestRenderer.ReactTestRenderer) {
  const animatedViews = renderer.root.findAll(
    (node) => String(node.type) === "Reanimated.View",
  );
  const frameNode = animatedViews.find((node) => {
    ...
    return (
      styles.some(
        (style) =>
          style?.backgroundColor === "#05070A" && style?.overflow === "hidden",
      ) &&
  ...
  const searchSheet = renderer.root
    ...
    .find((node) =>
      flattenStyle(node.props.style).some(
        (style) => style?.backgroundColor === "#1F2229",
      ),
    );
  return getTransformValue(
    flattenStyle(searchSheet?.props.style),
    "translateY",
  ) as number;
}
...
describe("DetailScreen", () => {
  it("renders every media slide in a paging gallery", () => {
    ...
    const mediaScroll = renderer!.root
  ...
  it("renders each typed media item as image or video when carousel media is mixed", () => {
      ...
      mediaItems: [
        {
          ...
          mediaType: "IMAGE",
          thumbnailUrl: "https://cdn.example.com/slide-1.jpg",
        },
        {
          url: "https://cdn.example.com/slide-2-video.mp4",
          mediaType: "VIDEO",
          thumbnailUrl: "https://cdn.example.com/slide-2-cover.jpg",
        },
        {
          url: "https://cdn.example.com/slide-3.jpg",
          mediaType: "IMAGE",
          thumbnailUrl: "https://cdn.example.com/slide-3.jpg",
        },
      ],
      mediaType: "VIDEO",
    };
    ...
    const videoViews = renderer!.root.findAll(
    ...
    expect(
      videoMock.players.some(
        (player) =>
          player.source.uri === "https://cdn.example.com/slide-2-video.mp4",
      ),
    ).toBe(true);
  ...
  it("does not move to the next reel from a summary bottom-edge pull-up while the sheet is open", () => {
          ...
          route={
            {
              key: "Detail",
              name: "Detail",
              params: { groupBuy: queryMock.groupBuys[0] },
            } as any
          }
          navigation={
            { goBack: vi.fn(), addListener: vi.fn(() => () => {}) } as any
    ...
    act(() => {
      findSummaryScrollGesture(renderer!).onEnd({
        translationY: -100,
        velocityY: -800,
      });
    });
  ...
  it("opens and closes the summary bottom sheet from the reel preview", () => {
    ...
    const raisedMediaStage = renderer!.root
      ...
      .find((node) => {
        ...
        return styles.some((item) => {
          ...
          return (
            styleObj.borderRadius !== undefined && styleObj.height !== undefined
          );
        });
      });
    ...
    const expectedSummarySheetHeight = Math.max(
      280,
      Math.min(844 - 24 - spacing.xl, 844 * 0.58),
    );
    const mediaOpenTop = 24 + spacing.sm;
    const expectedMediaBottom = 844 - expectedSummarySheetHeight;
    const expectedContentScale = Math.min(
      1,
      Math.max((390 - 96) / 390, (expectedMediaBottom - mediaOpenTop) / 844),
    );
    const summarySheet = renderer!.root
      .findAll((node) => String(node.type) === "Reanimated.View")
      .find((node) =>
        flattenStyle(node.props.style).some(
          (style) => style?.backgroundColor === "#111417",
        ),
      );
    const topBar = renderer!.root
      ...
      .find((node) => {
        const styles = flattenStyle(node.props.style);
        return (
          styles.some((style) => style?.justifyContent === "space-between") &&
          getStyleValue(styles, "opacity") !== undefined
        );
      });

    expect(
      getStyleValue(flattenStyle(summarySheet?.props.style), "maxHeight"),
    ).toBeCloseTo(expectedSummarySheetHeight, 1);
    expect(mediaFrame.top).toBeCloseTo(mediaOpenTop, 1);
    expect(mediaFrame.bottom).toBeCloseTo(expectedMediaBottom, 1);
    expect(mediaFrame.height).toBeCloseTo(
      expectedMediaBottom - mediaOpenTop,
      1,
    );
    ...
    expect(mediaFrame.contentTranslateY).toBeCloseTo(
      (mediaFrame.height - 844) / 2,
      1,
    );
    ...
    expect(
      renderer!.root.find(
        (node) =>
          String(node.type) === "Pressable" &&
          node.props.accessibilityLabel === "요약 닫기",
      ),
    ).toBeDefined();
    ...
    const purchaseButton = renderer!.root.findAll(
      (node) =>
        String(node.type) === "Pressable" &&
        node.props.accessibilityLabel === "구매 링크",
    )[0];
  ...
  it("does nothing when pressing a purchase link for an expired group buy", () => {
          ...
          route={
            {
              key: "Detail",
              name: "Detail",
              params: { groupBuy: expiredGroupBuy },
            } as any
          }
          navigation={
            { goBack: vi.fn(), addListener: vi.fn(() => () => {}) } as any
    ...
    const expiredButton = renderer!.root.findAll(
      (node) =>
        String(node.type) === "Pressable" &&
        node.props.accessibilityLabel === "마감된 공구",
    )[0];
  ...
  it("keeps the summary sheet open when the inner scroll only reaches the top during the same drag", () => {
    ...
    act(() => {
      ...
      scrollView.props.onContentSizeChange(0, 360);
      scrollView.props.onScrollBeginDrag({
        nativeEvent: { contentOffset: { y: 140 } },
      });
      scrollView.props.onScroll({ nativeEvent: { contentOffset: { y: 0 } } });
    });
  ...
  it("does not replace the preloader when there is no preload source", () => {
    ...
    act(() => {
      renderer = TestRenderer.create(
        <ReelVideoPreloader items={[baseGroupBuy]} activeIndex={0} enabled />,
      );
    });
...
describe("DetailScreen video playback", () => {
  ...
  it("renders VideoView for videoUrl even when the URL has no video extension", () => {
    ...
    expect(
      images.some((image) => image.props.source?.uri === groupBuy.thumbnailUrl),
    ).toBe(true);
    expect(videoViews[0]?.props.contentFit).toBe("contain");
  ...
  it("shows playback and mute controls when the video is tapped", () => {
    ...
    const getTotalPlayCalls = () =>
      videoMock.players.reduce(
        (sum, item) => sum + item.play.mock.calls.length,
        0,
      );
    const getTotalPauseCalls = () =>
      videoMock.players.reduce(
        (sum, item) => sum + item.pause.mock.calls.length,
        0,
      );
    const videoViews = renderer!.root.findAll(
      (node) => String(node.type) === "VideoView",
    ...
    const muteButton = renderer!.root.find(
      (node) =>
        String(node.type) === "Pressable" &&
        node.props.accessibilityLabel === "음소거",
    );
