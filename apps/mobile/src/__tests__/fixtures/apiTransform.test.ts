...
describe('API 변환 로직 — 실제 PostgREST 응답 기반', () => {
  ...
  describe('fetchGroupBuys 매핑', () => {
    it('PostgREST 원본 응답을 GroupBuy[]로 정확히 변환한다', async () => {
      vi.mocked(postgrestGet).mockResolvedValue({ data: mappedGroupBuys, meta: undefined });
    ...
    it('mapPostgrestToApp가 camelCase로 변환한 필드를 정확히 매핑한다', async () => {
      vi.mocked(postgrestGet).mockResolvedValue({ data: [mappedGroupBuys[0]], meta: undefined });
    ...
    it('중첩된 rawPostId.influencerId에서 username을 추출한다', async () => {
      ...
      expect(result[0].rawPost.influencer.instagramUsername).toBe('some_influencer');
      expect(result[0].rawPost.postUrl).toBe('https://instagram.com/p/TESTPOST001');
    });

    it('rawPostId가 null이어도 크래시하지 않는다', async () => {
      const brokenItem = { ...mappedGroupBuys[0], rawPostId: null };
      vi.mocked(postgrestGet).mockResolvedValue({ data: [brokenItem], meta: undefined });
    ...
    it('null 필드는 null 그대로 유지한다', async () => {
      ...
      vi.mocked(postgrestGet).mockResolvedValue({ data: [nullItem], meta: undefined });
  ...
  describe('fetchInfluencers 매핑', () => {
    it('mapPostgrestToApp 변환 후 Influencer[]를 그대로 반환한다', async () => {
      vi.mocked(postgrestGet).mockResolvedValue({ data: mappedInfluencers, meta: undefined });
  ...
  describe('mapPostgrestToApp 변환 검증', () => {
    it('snake_case 키를 camelCase로 재귀 변환한다', () => {
      const mapped = mapPostgrestToApp(rawGroupBuysResponse[0]) as Record<string, unknown>;
      ...
      expect((mapped as any).rawPostId.influencerId).toHaveProperty('instagramUsername');
    ...
    it('influencers 응답도 camelCase로 변환된다', () => {
      const mapped = mapPostgrestToApp(rawInfluencersResponse[0]) as Record<string, unknown>;
  ...
  describe('fallbackGroupBuys', () => {
    it('API 실패 시 사용할 수 있는 비어있지 않은 fallback 데이터가 있다', () => {
      ...
      expect(fallbackGroupBuys[0].rawPost.influencer.instagramUsername).toBeTruthy();
    });
