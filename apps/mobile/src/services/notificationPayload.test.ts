...
describe("notification payload", () => {
  it("round-trips a bounded canonical group-buy URL", () => {
    ...
    expect(url).toBe("gongguwish://group-buy/group-buy-1");
  ...
  it("uses a valid URL or falls back to groupBuyId", () => {
    expect(
      notificationDataToUrl({
        url: "gongguwish://group-buy/deal%20one",
        groupBuyId: "ignored",
      }),
    ).toBe("gongguwish://group-buy/deal%20one");
    expect(notificationDataToUrl({ groupBuyId: "deal-two" })).toBe(
      "gongguwish://group-buy/deal-two",
    );
  });
  ...
  it("rejects external schemes, wrong hosts, nested paths, and malformed data", () => {
    ...
    expect(
      parseGroupBuyNotificationUrl("gongguwish://settings/group-buy-1"),
    ).toBeNull();
    expect(
      parseGroupBuyNotificationUrl("gongguwish://group-buy/a/b"),
    ).toBeNull();
