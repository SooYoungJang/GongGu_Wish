...
test.describe("대시보드", () => {
  test("대시보드 핵심 섹션이 렌더링된다", async ({ adminPage: page }) => {
    ...
    await expect(page.locator("text=검수 대기")).toBeVisible({ timeout: 10000 });
    await expect(page.locator("text=최근 승인 공구")).toBeVisible();
  });
  ...
  test("검수 대기로 이동 버튼이 작동한다", async ({ adminPage: page }) => {
    ...
    if (await openButton.isVisible().catch(() => false)) {
      ...
      await expect(page.locator("text=위시 검수")).toBeVisible({ timeout: 5000 });
    }
  });
});
