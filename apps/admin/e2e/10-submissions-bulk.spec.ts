...
test.describe("검수 벌크 액션", () => {
  ...
  test("선택된 항목 수가 표시된다", async ({ adminPage: page }) => {
    await page.locator('button:has-text("검수")').first().click();
    await expect(page.locator("text=/\d+개 선택됨/")).toBeVisible({ timeout: 10000 });
  });
  ...
  test("체크박스로 항목 선택 시 카운트가 변경된다", async ({ adminPage: page }) => {
    await page.locator('button:has-text("검수")').first().click();
    await page.waitForTimeout(1000);
    const checkboxes = page.locator('.mobile-check input[type="checkbox"]:not(:disabled)');
    const count = await checkboxes.count();
    if (count > 0) {
      await checkboxes.first().check();
      await expect(page.locator("text=/[1-9]\d*개 선택됨/")).toBeVisible({ timeout: 3000 });
    }
  });
});
