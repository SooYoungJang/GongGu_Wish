-- GON-258: canonical group-buy ranking aggregation.
-- Keep get_popular_group_buys for older clients; this is an additive v2 RPC.

CREATE OR REPLACE FUNCTION public.get_group_buy_rankings(
...
AS $$
  ...
  paged AS (
    ...
    WHERE cursor_group_buy_id IS NULL
      OR (
        sort_filter IN ('popular', 'rising')
        AND cursor_numeric IS NOT NULL
        AND (
          CASE WHEN sort_filter = 'popular' THEN score ELSE score_delta END < cursor_numeric
          OR (
            CASE WHEN sort_filter = 'popular' THEN score ELSE score_delta END = cursor_numeric
            AND group_buy_id > cursor_group_buy_id
          )
        )
      )
      OR (
        sort_filter = 'deadlineSoon'
        AND (
          COALESCE(end_date, 'infinity'::timestamp) > COALESCE(cursor_timestamp, 'infinity'::timestamp)
          OR (
            COALESCE(end_date, 'infinity'::timestamp) = COALESCE(cursor_timestamp, 'infinity'::timestamp)
            AND group_buy_id > cursor_group_buy_id
          )
      ...
      OR (
        ...
        AND (
          ...
          OR (created_at = cursor_timestamp AND group_buy_id > cursor_group_buy_id)
        )
      )
  )
  SELECT
...
GRANT EXECUTE ON FUNCTION public.get_group_buy_rankings(text, text, text, integer, numeric, timestamp, text)
