...
const PRICE_RE =
  /(?:공구가|판매가|가격|특가|정가)?\s*[:：]?\s*([0-9]{1,3}(?:[,\s][0-9]{3})+|[0-9]{3,7})\s*(?:원|₩|KRW)(?=\s|$|[,.!?])/gi;
...
function extractPriceKrw(caption: string) {
  const matches = Array.from(caption.matchAll(PRICE_RE));
...
function extractBrandFromXLine(lines: string[]) {
  const xLine = lines.find(
    (line) =>
      !URL_RE.test(line) && /^.{2,30}?\s*(?:x|X|×)\s*.{2,50}$/.test(line),
  );
...
function extractProductName(lines: string[], brandName?: string) {
  const candidates = lines.filter((line) => {
    ...
    if (/^(댓글|문의|구매|링크|프로필|스토리|DM|디엠)\b/i.test(line))
      return false;
  ...
  const gongguLine =
    candidates.find(
      (line) => /공구/.test(line) && !/일정\s*(?:OPEN|오픈)?$/i.test(line),
...
export function parseSubmissionCaption(
  ...
  if (
    brandName &&
    productName &&
    /(?:공구\s*일정|일정\s*(?:OPEN|오픈)|브랜드들과|공구\s*일정\s*오픈)/i.test(
      productName,
    )
  ) {
  ...
  return {
    ...
    priceKrw: extractPriceKrw(caption),
  };
}
